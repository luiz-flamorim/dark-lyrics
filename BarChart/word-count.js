...
function countWords(data) {
    let parsedSongNames = getSongNames(data)
    ...
    for (let i = 0; i < normalization.length; i++) {
        if (!wordMap.has(normalization[i])) {
            wordMap.set(`${normalization[i]}`, 1)
        } else {
            wordMap.set(`${normalization[i]}`, wordMap.get(`${normalization[i]}`) + 1)
        }
    }
...
function normaliseLanguages(string) {
