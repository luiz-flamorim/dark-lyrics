...
function createTimeline(data) {
    ...
    function mouseClick(d) {
        ...
        title.innerHTML = d3.select(this).data()[0][0]
        contentDiv.appendChild(title)

        let subTitle = document.createElement('p')
        subTitle.setAttribute('class', `popup-sub-title`)
        let totalAlbums = d3.select(this).data()[0][1]
        let released = ' albums released'
        if (totalAlbums === 1) {
            released = ' album released'
        }
        subTitle.innerHTML = totalAlbums + released
        ...
        let bluredBg = document.querySelector('.modal-bg')
    ...
    function addAlbumList(year) {
        ...
        albums.forEach(album => {
            let element = `<li class="listItem">${normalise(album)}</li>`
