...
function createArch(data) {

    let dataMap = new Map()

    for (let i = 0; i < data.length; i++) { // bands
        let band = normalise(data[i].bandName)

        for (let j = 0; j < data[i].bandAlbums.length; j++) { // albums

            for (let k = 0; k < data[i].bandAlbums[j].albumSongs.length; k++) { // songs
                let song = normalise(data[i].bandAlbums[j].albumSongs[k].songEn)

                if (!(dataMap.has(song))) {
                    dataMap.set(`${song}`, [`${band}`])
                } else {
                    let bandArray = dataMap.get(`${song}`)
                    bandArray.push(`${band}`)
                    dataMap.set(`${song}`, bandArray)
                }
            }
        }
    }
